...
renda_de_setor_p_grade_muni_1 <- function(ano, sigla_muni) {
  ...
  out_file1 <- sprintf("tmp_grade_renda_cor_%s_%s.rds", sigla_muni, ano)
  out_file2 <- sprintf("grade_renda_cor_%s_%s.rds", sigla_muni, ano)
  
  if (out_file1 %nin% list.files(subfolder7A) & out_file2 %nin% list.files(subfolder7A)){
    ...
    # Status message
    etapa1 <- Sys.time()
    message('\n', etapa1, ' - 1-3: Começando cálculo de área geral e st_intersect() em grade_corrigida em: ', sigla_muni, '\n')
    ...
    write_rds(ui, sprintf("%s/%s", subfolder7A, out_file1), compress = 'gz')
    
    # Status message
    etapa4 <- Sys.time()
    message('\n', sigla_muni, ' temporário finalizado. Tempos:\n')
    message('Etapa 1: ', round(etapa2-etapa1), '\nEtapa2: ', round(etapa3-etapa2), '\nEtapa 3: ', round(etapa4-etapa3), '\n')
    beep(sound = 4)
    
  } else {
    message('Arquivo temporário para a cidade ', sigla_muni, " já existe, pulando...\n")
  }
...
renda_de_setor_p_grade_muni_2 <- function(ano, sigla_muni) {
  ...
  if (out_file1 %in% list.files(subfolder7A) & out_file2 %nin% list.files(subfolder7A)){
    # Ler arquivo temporário, resultado do processo anterior
    path_ui <- sprintf("%s/%s", subfolder7A, out_file1)
    ...
    # Salvar em disco
    write_rds(ui_fim_sf, sprintf("%s/%s", subfolder7A, out_file2), compress = 'gz')
    
    # Status message
    etapa3 <- Sys.time()
    message('\n', sigla_muni, ' finalizado. Tempos:\nEtapa 1: ', round(etapa2-etapa1), '\nEtapa2: ', round(etapa3-etapa2), '\n')
    beep(sound = 4)
