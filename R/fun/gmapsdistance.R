...
#' #
#' # results
gmapsdistance = function(origin, destination, combinations = "all", mode, transit_mode = "None", key = get.api.key(), shape = "wide",
                         avoid = "",
                         departure = "now", dep_date = "", dep_time = "",
                         traffic_model = "None", # None or empty?
  ...
  # If transit mode of transportation not recognized:
  if (!(transit_mode %in% c("bus",  "subway",  "train",  "tram", "rail", "None"))) {
    stop(
      "Transit mode of transportation not recognized. Transit mode should be one of ",
      "'bus', 'subway', 'train', 'tram', 'rail' "
    )
  }
  else if (transit_mode == "None") {
    transit_mode_string = ''
  } else {
    if (is.null(key)){
      stop('You need to provide a Google Maps API key if you want to use `traffic_model`. Use: `set.api.key(YOUR_KEY)`.')
    }
    transit_mode_string = paste0("&transit_mode=", transit_mode)
  }
