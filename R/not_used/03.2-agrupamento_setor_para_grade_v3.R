...
renda_de_setor_p_grade_muni_1 <- function(ano, sigla_muni) {
  ...
  if (out_file1 %nin% list.files(subfolderY) & out_file2 %nin% list.files(subfolderY) & out_file3 %nin% list.files(subfolderY)){
    ...
    write_rds(grade_corrigida, sprintf("%s/%s", subfolderY, out_file1), compress = 'gz')
...
renda_de_setor_p_grade_muni_2 <- function(ano, sigla_muni) {
  ...
  if (out_file1 %in% list.files(subfolderY) & out_file2 %nin% list.files(subfolderY) & out_file3 %nin% list.files(subfolderY)){
    # Caminho para os arquivos
    path_setor <- sprintf("%s/setores_agregados_%s_%s.rds", subfolder4, sigla_muni, ano)
    path_grade_corrigida <- sprintf("%s/%s", subfolderY, out_file1)
    ...
    # Salvar arquivo temporário em disco
    write_rds(ui, sprintf("%s/%s", subfolderY, out_file2), compress = 'gz')
...
renda_de_setor_p_grade_muni_3 <- function(ano, sigla_muni) {
  ...
  if (out_file2 %in% list.files(subfolderY) & out_file3 %nin% list.files(subfolderY)){
    # Ler arquivo temporário, resultado do processo anterior
    path_ui <- sprintf("%s/%s", subfolderY, out_file2)
    ...
    # Salvar em disco
    write_rds(ui_fim_sf, sprintf("%s/%s", subfolderY, out_file3), compress = 'gz')
