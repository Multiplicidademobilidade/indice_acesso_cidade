...
renda_de_setor_p_grade_muni_1 <- function(ano, sigla_muni) {
  ...
  if (out_file1 %nin% list.files(subfolderY) & out_file2 %nin% list.files(subfolderY)){
    ...
    write_rds(ui, sprintf("%s/%s", subfolderY, out_file1), compress = 'gz')
...
renda_de_setor_p_grade_muni_2 <- function(ano, sigla_muni) {
  ...
  if (out_file1 %in% list.files(subfolderY) & out_file2 %nin% list.files(subfolderY)){
    # Ler arquivo temporário, resultado do processo anterior
    path_ui <- sprintf("%s/%s", subfolderY, out_file1)
    ...
    # Salvar em disco
    write_rds(ui_fim_sf, sprintf("%s/%s", subfolderY, out_file2), compress = 'gz')
